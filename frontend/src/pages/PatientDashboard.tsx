...
export default function PatientDashboard() {
  ...
  const [form, setForm] = useState({ nom: "", prenom: "", dateNaissance: "", sexe: "", telephone: "", adresse: "", profession: "", allergies: "", antecedentsMedicaux: "", consentGiven: false })
  const { user } = useAuth()
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowCreate(false)
      setForm({ nom: "", prenom: "", dateNaissance: "", sexe: "", telephone: "", adresse: "", profession: "", allergies: "", antecedentsMedicaux: "", consentGiven: false })
    } catch (err) {
      console.error(err)
    }
  }
      ...
      <div className="patient-grid">
        {records.map((record) => (
          <div key={record.id} className="card patient-card" onClick={() => navigate(isPatient ? `/share/${record.id}` : `/patients/${record.id}`)}>
            ...
            {record.telephone && <p>Tél: {record.telephone}</p>}
            <p className="consent-badge">{record.consentGiven ? "✓ Consentement" : "✗ Pas de consentement"}</p>
          </div>
        ))}
